...
var AppManager = exports.AppManager = function() {
  ...
  self.on('configUpdate', function(app) {
    self.queue.prepare.push(app);
    self.runQueue('prepare');
  });

  self.on('fetchComplete', function() {
    self.runQueue('prepare');
  });
  
  var fetchTask = function(fn) {
    self.fetch(function(fetchCount) {
      if (fetchCount > 0) {
        self.emit('fetchComplete');
      }
      fn();
    });
  };
  
  this.scheduler = new Scheduler(fetchFrequency, fetchTask);
  
};
AppManager.prototype = Object.create(EventEmitter.prototype);

AppManager.prototype.runQueue = function(name) {
  var self = this;
...
AppManager.prototype.redeploy = function(app) {
  ...
  this.runQueue('prepare');
};

AppManager.prototype.lookup = function(reposUrl) {
  var apps = [];
  var self = this;
...
AppManager.prototype.fetch = function(fn) {
  var self = this;
  ...
  fs.readdir(homePath, function(err, configFiles) {
    ...
    var checkDeleted = function() {
      Object.keys(self.apps).forEach(function(user) {
        var reposApps = self.apps[user];
        _(reposApps).each(function(app, appName) {
          ...
          fs.readFile(file, 'utf8', function(err, rawConf) {
            ...
            apps.forEach(function(app) {
              ...
              if (!existing) {
                ...
                newApps[user][app.name] = self.apps[user][app.name];
              }
            });
            complete();
          });
